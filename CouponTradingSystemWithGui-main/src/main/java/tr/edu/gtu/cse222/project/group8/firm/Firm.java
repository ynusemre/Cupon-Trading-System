...
public class Firm implements FileObj{
	...
	public Firm(String firmName){
		this.id = FileHelper.returnDataFieldByName(new File(CouponTradingSystem.firmFile), firmName, 1);
                System.out.println("SSSSSSSSSSSSSSSSSSSS    " + id);
	...
	public TreeSet<CouponStock> getonSaleCoupons() {
        ...
        public Coupon addNewCoupon(String couponName, String expireDate, int stock) {
            ...
            Coupon newCoupon;
            try {
                newCoupon = new Coupon(couponName, this, Coupon.formatter.parse(expireDate), 0, 0);
                FileHelper.addLine(new File(CouponTradingSystem.couponsFile), newCoupon.toWrite());
                onSaleCoupons.add(new CouponStock(newCoupon, stock));
                return newCoupon;
	...
	public boolean removeCoupon(Coupon coupon) {
            ...
            //if(FileHelper.checkDataField(new File (CouponTradingSystem.couponsFile),coupon.getName(),6)) return false;
            ...
            while (it.hasNext()) {
                ...
                if (temp.coupon.equals(coupon)) {
                    ...
                    it.remove();
                    FileHelper.removeByID(new File(CouponTradingSystem.couponsFile), coupon.getId());
                    return true;
                }
            }
